...
//#region 加载排序集合
function loadSortArray(line) {
    var fromArray = new Array();
    var toArray = new Array();
    //分组
    $.each(line, function (index, key) {
        fromArray.push(key.from);
        toArray.push(key.to);
    });
    //求差
    var diff = difference(fromArray, toArray);
    var beginNodeCode = diff[0];
    if (!($.inArray(beginNodeCode, fromArray) > -1)) {
        beginNodeCode = diff[1];
    }
    var newArray = [];
    getClist(beginNodeCode, line, newArray);
    $.each(line, function (index, key) {
        var data = get(newArray, "to", key.to);
        if (!data && key.to === diff[1]) {
            newArray.push(key);
        }
    });
    return newArray;
...
//获取指定节点信息
function getNodeinnerText(nodeDom, nodeCode) {
    var rejectNode = {
        innerText: null,
        rejectNodeCode: nodeCode
    };
    //分组
    $.each(nodeDom, function (index, key) {
        if (nodeCode === index) {
            rejectNode.innerText = key[0].innerText;
        }
    });
    return rejectNode;
}
